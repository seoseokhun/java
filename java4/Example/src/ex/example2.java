...
public class example2 {
	...
	public static void main(String[] args) {
		/*
		 * 회원정보 DB가 있는 상황입니다.
		 * 해당 정보 중에서 naver.com 메일 주소를 사용하는 고객인원수를 출력합니다.
		...
		ul.setter("naver.com");
		int total = ul.getter();
		System.out.println("naver.com은 총 "+total+" 명이 있습니다.");
...
class user_list extends basic{
	...
	public void setter(String email) {
		...
		for(int f =0; f<this.ea;f++) {
			/* 원시형태의 배열사항으로 카운팅을 하게 됨 indexOf -1(없음) -1외 모든 정수는 값이 있음 */
			if(this.arr[f][5].indexOf(email)!=-1) {
				this.total += 1;
			}
			
			/* ArrayList : 로 생성시 메모리 할당 오버되는 상황이 발생 할 수 있음 */
